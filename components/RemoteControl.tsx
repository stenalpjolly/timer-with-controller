...
export const RemoteControl: React.FC<RemoteControlProps> = ({ onBack }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 p-6">
        <div className="w-full max-w-md bg-slate-800 rounded-3xl p-8 shadow-2xl border border-slate-700">
          ...
          <p className="text-slate-400 mb-6">Enter the 4-letter code displayed on the main screen.</p>
          ...
          <form onSubmit={handleConnect} className="space-y-4">
            <input 
              ...
              placeholder="ABCD"
              className="w-full bg-slate-950 text-white text-5xl font-black p-6 rounded-2xl border-4 border-slate-700 focus:border-indigo-500 focus:outline-none text-center tracking-widest uppercase placeholder:text-slate-800"
              autoFocus
            />
            ...
            <button 
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              {isConnecting ? <Loader2 className="animate-spin" /> : 'Connect to Display'}
            </button>
          </form>
        </div>
      </div>
  ...
  // Loading State
  if (isConnected && !syncedState) {
      return (
        <div className="flex flex-col items-center justify-center h-screen bg-slate-950 text-white">
            <Loader2 className="animate-spin mb-4" size={48} />
            <div className="font-bold">Syncing with display...</div>
            <button onClick={() => { connRef.current?.close(); setIsConnected(false); }} className="mt-8 text-slate-500 hover:text-white text-sm">
                Cancel
            </button>
        </div>
      );
  ...
  const isIdle = syncedState?.status === TimerStatus.IDLE;
    ...
    <div className="flex flex-col h-screen bg-slate-950 text-white">
      ...
      <div className="p-4 bg-slate-900 flex justify-between items-center border-b border-slate-800 shrink-0">
        <div className="flex items-center gap-2 text-green-400 font-bold text-sm">
          <Wifi size={16} /> Connected to {pairingCode}
        </div>
        <button onClick={() => { connRef.current?.close(); setIsConnected(false); }} className="text-slate-500 hover:text-white text-sm">
          Disconnect
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
          <div className={clsx("text-6xl font-black tabular-nums tracking-tighter transition-colors mb-2", 
        ...
        {isIdle ? (
                 ...
                 <div>
                    <div className="flex items-center gap-4">
                        <button 
                           ...
                           className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center text-xl font-bold hover:bg-slate-700 active:scale-95"
                        >
                            -
                        </button>
                        <div className="flex-1 bg-slate-950 p-3 rounded-xl border border-slate-700 text-center text-3xl font-black">
                    ...
                    <div className="flex gap-2 mt-3 overflow-x-auto pb-2 no-scrollbar">
                        {[5, 10, 15, 20, 30, 45, 60].map(m => (
                            <button 
                                ...
                                className={clsx("px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition-colors",
            ...
            <div className="grid grid-cols-2 gap-4">
                <button 
                    ...
                    className={clsx("py-6 rounded-2xl font-black text-xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg",
                ...
                <button 
                    ...
                    className="py-6 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-2xl font-bold text-xl flex items-center justify-center gap-3 transition-all active:scale-95 border border-slate-700"
                >
                    <RotateCcw /> RESET
                </button>
            </div>
